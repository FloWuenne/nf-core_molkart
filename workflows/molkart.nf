...
include { GRIDLINE_FORGE_DUPLICATEFINDER } from '../modules/local/gridline_forge_duplicatefinder/main'
...
workflow MOLKART {
    ...
    image_tuple = ch_samplesheet.map { meta, nuclear, _spots, _membrane ->
        [meta + [stain: '0'], nuclear]
    }

    spot_tuple = ch_samplesheet.map { meta, _nuclear, spots, _membrane ->
        [meta, spots]
    }

    //
    // MODULE: Run grid-line filling with GridlineForge
    //
    gap_filler_in = membrane_tuple.mix(image_tuple)
    // gap filler input contains both membrane and nuclear images
    ...
    //
    clahe_in = params.skip_gridline_filling ? gap_filler_in : gap_filler_out
    CLAHE(clahe_in)
    ...
    grouped_map_stack
        ...
        .set { nuclear_only }
    // for segmentation options that only accept one channel
    ...
    if (params.create_training_subset) {
        ...
        //
        // MODULE: GridlineForge DuplicateFinder
        //
        // Filter out potential duplicate spots from the spots table
        GRIDLINE_FORGE_DUPLICATEFINDER(spot_tuple)
        ch_versions = ch_versions.mix(GRIDLINE_FORGE_DUPLICATEFINDER.out.versions)

        qc_spots = params.skip_duplicatefinder ? spot_tuple : GRIDLINE_FORGE_DUPLICATEFINDER.out.marked_dups_spots
        //
        ...
        if (params.segmentation_method.split(',').contains('mesmer')) {
            DEEPCELL_MESMER(
                ...
                grouped_map_stack.map {
                    it[2] == null ? [[:], []] : tuple(it[0], it[2])
                },
            )
            ch_versions = ch_versions.mix(DEEPCELL_MESMER.out.versions)
            segmentation_masks = segmentation_masks.mix(
                DEEPCELL_MESMER.out.mask.combine(Channel.of('mesmer'))
            )
        }
        //
        ...
        if (params.segmentation_method.split(',').contains('stardist')) {
            ...
            segmentation_masks = segmentation_masks.mix(
                STARDIST.out.mask.combine(Channel.of('stardist'))
            )
        }
        //
        ...
        if (params.segmentation_method.split(',').contains('cellpose')) {
            ...
            segmentation_masks = segmentation_masks.mix(
                CELLPOSE.out.mask.combine(Channel.of('cellpose'))
            )
        }
        //
        ...
        if (params.segmentation_method.split(',').contains('ilastik')) {
            ...
            ilastik_in
                .join(ILASTIK_PIXELCLASSIFICATION.out.output)
            ...
            segmentation_masks = segmentation_masks.mix(
                ILASTIK_MULTICUT.out.out_tiff.combine(Channel.of('ilastik'))
            )
        }
        segmentation_masks
            .map { meta, mask, segmentation ->
        ...
        qc_spots
            ...
            .map { meta, spots_table, mask, segmethod ->
                def new_meta = meta.clone()
        ...
        SPOT2CELL.out.cellxgene_table
            .combine(
                MASKFILTER.out.filtered_qc,
                by: 0
            )
            .map { meta, quant, filterqc ->
    ...
    if (params.create_training_subset) {
        ch_multiqc_files = ch_multiqc_files.mix(
            MOLKARTQCPNG.out.png_overview.collectFile(name: "crop_overview.png", storeDir: "${params.outdir}/multiqc")
        )
        ch_multiqc_files = ch_multiqc_files.mix(
            CROPHDF5.out.crop_summary.map { it[1] }.collectFile(name: 'crop_overview.txt', storeDir: "${params.outdir}/multiqc")
        )
    }
    else {
        ch_multiqc_files = ch_multiqc_files.mix(
            MOLKARTQC.out.qc.map { it[1] }.collectFile()
        )
    }
